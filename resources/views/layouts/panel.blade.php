@extends('layouts.app')

@push('head')
    <link rel="stylesheet" href="{{ asset('css/postulation-list.css') }}">
    <link rel="stylesheet" href="{{ asset('css/postulation-cover.css') }}">
    <link rel="stylesheet" href="{{ asset('css/panel.css') }}">
@endpush

@section('content')
    <div class="container">
        @if(!isset($welcome) || $welcome !== false)
            <h1 class="title_panel">Hola, {{ Auth::user()->full_name }}</h1>
        @endif

        @if(!isset($nav) || $nav !== false)
            <nav class="nav">
                <ul>
                    <li>
                        <a class="button-link {{ request()->routeIs('panel') ? 'button-link-active' : '' }}" href="{{ route('panel') }}">
                            <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z"></path>
                            </svg>
                            Postulaciones
                        </a>
                    </li>
                    <li>
                        <a class="button-link {{ request()->routeIs('panel.profile') ? 'button-link-active' : '' }}" href="{{ route('panel.profile') }}">
                            <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"></path>
                            </svg>
                            Perfil
                        </a>
                    </li>
                    <li>
                        <a class="button-link {{ request()->routeIs('panel.documents.upload') ? 'button-link-active' : '' }}" href="{{ route('panel.documents.upload') }}">
                            <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 8.25H7.5a2.25 2.25 0 0 0-2.25 2.25v9a2.25 2.25 0 0 0 2.25 2.25h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25H15m0-3-3-3m0 0-3 3m3-3V15"></path>
                              </svg>
                            Cargar documento
                        </a>
                    </li>
                </ul>
                <ul>
                    <li>
                        <a class="button-link" href="{{ route('logout') }}">
                            <svg data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0 3-3m0 0-3-3m3 3H9"></path>
                            </svg>
                        </a>
                    </li>
                </ul>
            </nav>
        @endif
    </div>

    @yield('section')
@endsection
